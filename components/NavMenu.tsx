...
import { Menu, Home, LayoutGrid, Flag, Clock, Target, HelpCircle } from 'lucide-react';

interface NavMenuProps {
  onNavigate: (page: 'home' | 'builder' | 'setpieces' | 'articles' | 'minutes' | 'drills' | 'faq') => void;
  currentPage: 'home' | 'builder' | 'setpieces' | 'articles' | 'minutes' | 'drills' | 'faq';
}
...
export const NavMenu: React.FC<NavMenuProps> = ({ onNavigate, currentPage }) => {
  ...
  const handleNav = (page: 'home' | 'builder' | 'setpieces' | 'articles' | 'minutes' | 'drills' | 'faq') => {
    onNavigate(page);
    setIsOpen(false);
  };
    ...
    <div className="relative" ref={menuRef}>
      ...
      {isOpen && (
        <div className="absolute top-full left-0 mt-2 w-56 bg-slate-900 border border-slate-700 rounded-xl shadow-2xl z-[100] overflow-hidden animate-in fade-in zoom-in-95 duration-200">
          <div className="py-2">
            <button type="button" onClick={() => handleNav('home')} className={itemClass('home')}>
              <Home size={16} />
              Homepage
            </button>
            <button type="button" onClick={() => handleNav('builder')} className={itemClass('builder')}>
              <LayoutGrid size={16} />
              Lineup Builder
            </button>
            <button type="button" onClick={() => handleNav('setpieces')} className={itemClass('setpieces')}>
              <Flag size={16} />
              Set Pieces
            </button>
            <button type="button" onClick={() => handleNav('drills')} className={itemClass('drills')}>
              <Target size={16} />
              Drills
            </button>
            <button type="button" onClick={() => handleNav('minutes')} className={itemClass('minutes')}>
              <Clock size={16} />
              Minutes Log
            </button>
            <button type="button" onClick={() => handleNav('faq')} className={itemClass('faq')}>
              <HelpCircle size={16} />
              FAQ
            </button>
          </div>
